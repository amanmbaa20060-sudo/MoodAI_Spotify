...
async function setMood(mood) {
  ...
  document.getElementById("home-content")?.classList.add("opacity-60");
  try {
    ...
    state.currentDropId = dropModule?.data?.drop_id || null;
    dismissTooltip();
    showToast(`Home recalibrated for ${formatMoodLabel(mood)}`);
    renderHome();
  } catch (error) {
    showToast("Could not update mood. Try again.");
  } finally {
    document.getElementById("home-content")?.classList.remove("opacity-60");
  }
}
...
function trackRow(track, { showHeardBefore = false, compact = false } = {}) {
    ...
    <div class="group flex items-center justify-between p-2 -mx-2 rounded-xl hover:bg-surface-container-high transition-colors">
      <div class="flex items-center gap-4 min-w-0 flex-1">
        ...
        <div class="flex flex-col min-w-0">
          ...
          <span class="text-caption font-caption text-on-surface-variant truncate">${escapeHtml(artists)}</span>
          ${track.reason_text ? `
...
function freshPickCard(track) {
  ...
  return `
    <div class="flex-shrink-0 w-44 snap-start group cursor-pointer">
      ...
      <p class="text-caption font-caption text-on-surface-variant truncate">${escapeHtml(artists)}</p>
    </div>`;
}
...
function artistCard(artist) {
  const genre = artist.top_genre
    ...
    : "";
  return `
    <div class="glass-card artist-glow p-4 rounded-xl flex flex-col items-center text-center transition-all duration-300">
...
function renderHome() {
  ...
  const activeMood = home.active_mood;
  ...
  const dropBody = dropTracks.length
    ? dropTracks.map((t) => trackRow(t, { showHeardBefore: true })).join("")
    : renderEmptyDrop(activeMood);

  document.getElementById("home-content").innerHTML = `
    <section class="space-y-3 relative">
      ${state.showFirstTimeTooltip ? `
        ...
        <div class="relative z-10 space-y-6">
          <div class="flex justify-between items-start gap-4">
            <div class="space-y-1">
              ...
              <p class="font-body-sm text-body-sm text-on-surface-variant">Today • ${formatDateLabel()} — ${dropTracks.length || 10} fresh tracks for your ${escapeHtml(formatMoodLabel(activeMood))} mood</p>
            </div>
  ...
  document.getElementById("mood-chips").innerHTML = options.map((m) => moodChip(m, m === activeMood)).join("");
  bindHomeEvents(dropTracks, freshTracks);
}

function bindHomeEvents(dropTracks, freshTracks) {
